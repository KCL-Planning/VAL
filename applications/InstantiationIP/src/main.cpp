...
void instantiatedOp::writeAll(ostream &o) {
  for (OpStore::const_iterator i = instOps.begin(); i != instOps.end(); ++i) {
    o << **i << "\n";
  };
};
...
void instantiatedOp::instantiate(const operator_ *op, const problem *prb,
                                 TypeChecker &tc) {
  FastEnvironment e(
      static_cast< const id_var_symbol_table * >(op->symtab)->numSyms());
  vector< vector< const_symbol * >::const_iterator > vals(
      op->parameters->size());
  vector< vector< const_symbol * >::const_iterator > starts(
      op->parameters->size());
  vector< vector< const_symbol * >::const_iterator > ends(
      op->parameters->size());
  ...
  while (vals[i] != ends[i]) {
     ...
     *		else
                    {
                            cout << "Killed\n" << op->name->getName() << "(";
                            for(var_symbol_list::const_iterator a =
     op->parameters->begin(); a != op->parameters->end();++a)
                            {
                                    cout << e[*a]->getName() << " ";
                            };
                            cout << ")\n";
...
void instantiatedOp::writePres(ostream &o) const { pres.write(this, o); };

void instantiatedOp::writeAdds(ostream &o) const { adds.write(this, o); };

void instantiatedOp::writeDels(ostream &o) const { dels.write(this, o); };
...
int main(int argc, char *argv[]) {
  ...
  for (operator_list::const_iterator os = an_analysis.the_domain->ops->begin();
       os != an_analysis.the_domain->ops->end(); ++os) {
