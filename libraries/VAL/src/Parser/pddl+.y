...
c_pref_con_goal_list :
	c_pref_con_goal_list c_pref_con_goal
		{$$=$1; if($2) {$1->push_back($2);};}
|	c_pref_con_goal
		{$$= new goal_list; if($1) {$$->push_back($1);};}
;
...
|   OPEN_BRAC AFTER c_name_or_goal_descriptor c_name_or_goal_descriptor CLOSE_BRAC
        {$$ = new constraint_goal(E_AFTER,$4,$3);}
|	OPEN_BRAC NAMETAG NAME c_goal_descriptor CLOSE_BRAC
		{$$ = 0;
		 current_analysis->goalnames[$3] = $4;
		 delete [] $3;}
;

c_name_or_goal_descriptor :
	c_goal_descriptor
	{$$ = $1;}
|	NAME
	{$$ = new named_goal(current_analysis->goalnames[$1]); delete [] $1;}
;
...
// Friends here
// Predicates next (NB: What about name clashes/name space here?)
                c_functions_def
// Supports finally
...
c_require_key :
   ...
   | CONSTRAINTS
                {$$ = E_CONSTRAINTS;}
   | SUPPLYDEMAND_REQ {$$ = E_SUPPLYDEMAND;}
   | NAME
