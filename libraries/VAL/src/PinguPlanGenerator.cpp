...
namespace VAL {
  ...
  void PinguPlanGen::visit_plan_step(plan_step *ps) {
    ...
    if (s == "fallfromstart") {
      ...
      //     cout << "Got a starter " << pingu << " at " <<
      //     (*(++(ps->params->begin())))->getName() << "\n";
      lastMoved = pingu;
    ...
    if (s == "unblock_by_bombing") {
      ...
      doThis("bomb", pos);
      blocked.erase(pos);
    } else if (s == "unblock_by_jumping") {
      string pos = (*(ps->params->begin()))->getName();
      doThis("jump", pos);
      blocked.erase(pos);
    } else if (s == "jump" || s == "block" || s == "float" || s == "bomb" ||
               s == "climb") {
      doAction(s, ps);
      ...
      if (position.find(pos) != position.end() &&
          position.find(sec) != position.end()) {
