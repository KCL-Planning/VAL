...
namespace VAL {
  ...
  bool TypeHierarchy::closure(Graph &gr, GI &gi, Nodes &vs, GI &gs,
                              const TypeRef *t) {
    if (gi == gr.end()) {
      return false;
    };
  ...
  struct matchPred {
    ...
    bool operator()(const pred_decl *pd) const { return p == pd->getPred(); };
  };
  ...
  struct matchOp {
    ...
    bool operator()(const operator_ *op) const { return f == op->name; };
  };

  struct matchSD {
    const operator_symbol *f;

    matchSD(const operator_symbol *fs) : f(fs){};

    bool operator()(const supplied_effect *se) const { return f == se->name; };
  };
   ...
   *
   * Therefore, subType(c - t1,v - t2) iff t1 - t2. Note that this relationship
   * might be via a path of transitive subset relations.
   *
   * v - (either t1 t2) is equivalent to v - (t1 U t2).
   * c - (either t1 t2) is not given a meaning, but we use the either field to
   * record multiple types for constants. Thus, c - t1 and c - t2 is legal and
   * is recorded internally using c - (either t1 t2). The meaning of the
   * multiple declarations is that c is of both types t1 and t2, so can be
   * substituted for any variable of either type. t - (either t1 t2) is also not
   * given a meaning (although the current parser will actually interpret it as
   * equivalent to "t - t1 t - t2").
   */

  bool TypeChecker::subType(const pddl_typed_symbol *tp1,
                            const pddl_typed_symbol *tp2) {
    ...
    if (tp1->type) {
      ...
      if (!tp1->either_types) {
        if (Verbose)
          *report << "Object with unknown type: " << tp1->getName() << "\n";
        TypeException te;
        throw(te);
      };

      // The situation is now complicated by the fact that variables and
      // constants must be treated differently. Constants have either types
      // representing conjunctions of types, while variables use them for
      // disjunctions.
      ...
      if (dynamic_cast< const const_symbol * >(tp1)) {
        // The following confirms that a constant is a subtype of a given type
        // by checking that one of its types is appropriate.
  ...
  bool TypeChecker::typecheckProposition(const proposition *p) {
    ...
    pred_decl_list::const_iterator prd =
        std::find_if(thea->the_domain->predicates->begin(),
                     thea->the_domain->predicates->end(), matchPred(p->head));
    if (prd == thea->the_domain->predicates->end()) {
      if (p->head->getName() == "=") return true;
      if (Verbose)
        *report << "Predicate " << p->head->getName() << " not found\n";
      return false;
    };
    int idx = 1;
  ...
  bool TypeChecker::typecheckFuncTerm(const func_term *ft) {
    if (!isTyped) return true;
    func_decl_list::const_iterator fd = std::find_if(
        thea->the_domain->functions->begin(),
        thea->the_domain->functions->end(), matchFunc(ft->getFunction()));
    const class_func_term *cft = dynamic_cast< const class_func_term * >(ft);

    if (fd == thea->the_domain->functions->end() && !cft) return false;
    if (cft) {
      // REMEMBER TO FIX THIS TO TYPECHECK PROPERLY
      return true;
    }
  ...
  bool TypeChecker::typecheckActionInstance(const plan_step *p) {
    ...
    operator_list::const_iterator op =
        std::find_if(thea->the_domain->ops->begin(),
                     thea->the_domain->ops->end(), matchOp(p->op_sym));
    if (op == thea->the_domain->ops->end()) {
      pc_list< supplied_effect * >::const_iterator op1 = std::find_if(
          thea->the_problem->initial_state->supplied_effects.begin(),
          thea->the_problem->initial_state->supplied_effects.end(),
          matchSD(p->op_sym));
      return op1 != thea->the_problem->initial_state->supplied_effects.end();
    }
    var_symbol_list::const_iterator arg = (*op)->parameters->begin();
    const_symbol_list::const_iterator e = p->params->end();
    for (const_symbol_list::const_iterator i = p->params->begin(); i != e;
         ++i, ++arg) {
      if (!subType(*i, *arg)) {
  ...
  bool TypeChecker::typecheckExpression(const expression *exp) {
    if (!isTyped) return true;
    if (const binary_expression *be =
            dynamic_cast< const binary_expression * >(exp)) {
      return typecheckExpression(be->getLHS()) &&
             typecheckExpression(be->getRHS());
    };
    if (const uminus_expression *ue =
            dynamic_cast< const uminus_expression * >(exp)) {
  ...
  bool TypeChecker::typecheckGoal(const goal *g) {
    ...
    if (const constraint_goal *cg =
            dynamic_cast< const constraint_goal * >(g)) {
      bool b1 = true;
      if (cg->getTrigger()) {
    ...
    if (const conj_goal *cg = dynamic_cast< const conj_goal * >(g)) {
      if (cg->getGoals()->end() == std::find_if(cg->getGoals()->begin(),
                                                cg->getGoals()->end(),
                                                badchecker(this)))
        return true;
      return false;
    ...
    if (const disj_goal *dg = dynamic_cast< const disj_goal * >(g)) {
      if (dg->getGoals()->end() == std::find_if(dg->getGoals()->begin(),
                                                dg->getGoals()->end(),
                                                badchecker(this)))
        return true;
      return false;
    ...
    if (const imply_goal *ig = dynamic_cast< const imply_goal * >(g)) {
      return typecheckGoal(ig->getAntecedent()) &&
             typecheckGoal(ig->getConsequent());
    };
    ...
    if (const comparison *c = dynamic_cast< const comparison * >(g)) {
      return typecheckExpression(c->getLHS()) &&
             typecheckExpression(c->getRHS());
    };
    if (const constraint_goal *cg =
            dynamic_cast< const constraint_goal * >(g)) {
  ...
  bool TypeChecker::typecheckEffect(const effect *e) {
    ...
    if (const assignment *ass = dynamic_cast< const assignment * >(e)) {
      return typecheckFuncTerm(ass->getFTerm()) &&
             typecheckExpression(ass->getExpr());
    };
    return false;
  };
  ...
  bool TypeChecker::typecheckEffects(const effect_lists *es) {
    if (!isTyped) return true;
    return (es->add_effects.end() == std::find_if(es->add_effects.begin(),
                                                  es->add_effects.end(),
                                                  badchecker(this))) &&
           (es->del_effects.end() == std::find_if(es->del_effects.begin(),
                                                  es->del_effects.end(),
                                                  badchecker(this))) &&
           (es->forall_effects.end() == std::find_if(es->forall_effects.begin(),
                                                     es->forall_effects.end(),
                                                     badchecker(this))) &&
           (es->cond_effects.end() == std::find_if(es->cond_effects.begin(),
                                                   es->cond_effects.end(),
                                                   badchecker(this))) &&
           (es->assign_effects.end() == std::find_if(es->assign_effects.begin(),
                                                     es->assign_effects.end(),
                                                     badchecker(this))) &&
           (es->timed_effects.end() == std::find_if(es->timed_effects.begin(),
                                                    es->timed_effects.end(),
                                                    badchecker(this)));
  };
  ...
  bool TypeChecker::typecheckAction(const operator_ *act) {
    ...
    if (const durative_action *da =
            dynamic_cast< const durative_action * >(act)) {
  ...
  bool TypeChecker::typecheckDerivationRule(const derivation_rule *d) {
    if (!isTyped) return true;
    if (Verbose)
      *report << "Type-checking derivation rule for "
              << (d->get_head()->head->getName()) << "\n";
    ...
    return i != thea->the_domain->predicates->end() &&
           typecheckGoal(d->get_body());
  };
  ...
  bool TypeChecker::typecheckProblem() {
    ...
    if (thea->the_problem->the_goal &&
        !typecheckGoal(thea->the_problem->the_goal)) {
    ...
    if (thea->the_problem->constraints &&
