...
namespace SAS {
  ...
  bool findLinkedValue(ValHolder *vh, const vector< ValueElement * > &vs) {
    ...
    for (PlaceHolder::const_iterator i = ph->begin(); i != ph->end(); ++i) {
      for (vector< ValueElement * >::const_iterator j = vs.begin();
           j != vs.end(); ++j) {
        //			if(!(*j)->getPS()) continue;
        ...
        /*			PropertyState::PSIterator p = (*i)->begin();
                                for(;p != (*i)->end();++p)
                                {
                                        PropertyState::PSIterator q =
           ((*j)->getPS())->begin(); for(;q != (*j)->getPS()->end();++q)
                                        {
    ...
    for (Range::const_iterator i = range.begin(); i != range.end(); ++i) {
      ...
      for (ValuesUnion::const_iterator j = i->begin(); j != i->end(); ++j) {
        ...
        for (; k != j->end(); ++k) {
          ...
          // If the object is not in this state, we have to move on to the next
          // state. Object is probably in this state if it has the corresponding
          // initial property. If it has that then we can extract the initial
          // proposition and the arguments we need. If it doesn't have it, then
          // we know it is not in this state.
          ...
          if (!wanted) {
            ...
            if (findLinkedValue(*k, vs)) {
              ...
              // a
              // value\n";
              tmpvals.push_back(new PlaceHolderElement(
                  dynamic_cast< const PlaceHolder * >(*k)));
            };
            continue;
          };
          ...
          if (got->args->size() == 1) {
            ...
            for (parameter_symbol_list::iterator pr = got->args->begin();
              ...
              if (c != wanted->aPosn()) vals.push_back(new ObElement(TOB(*pr)));
            };
          };
        };
        if (k == j->end() && !vals.empty()) {
          // Then we found we were in this state and so we can't be in any
          // other.
          break;
        };
        ps = 0;
      ...
      if (vals.empty()) {
        //			cout << "Really should consider another
        // possibility\n";
        vals = tmpvals;
      };
    ...
    for (vector< VAL::const_symbol * >::iterator i = cs.begin(); i != cs.end();
      ...
      if ((rngs[TOB(*i)] = constructValue(range, TOB(*i))).size() > 0) {
        SASOUTPUT {
          ...
          for_each(rngs[TOB(*i)].begin(), rngs[TOB(*i)].end() - 1,
                   ptrwriter< ValueElement >(cout, ","));
  ...
  set< PropertySpace * > relevant(pddl_type *tp) { return TA->relevant(tp); };

  void constructValues(Values &vals, const PropertyState *pst,
                       const pddl_type *pt, PropertySpace *psp) {
    for (PropertyState::PSIterator pp = pst->begin(); pp != pst->end(); ++pp) {
      if ((*pp)->familySize() == 1) {
        vals.push_back(new NullHolder(pt, pst, psp, *pp));
      } else {
        vector< pddl_type * > tps;
        vector< pddl_typed_symbol * >::const_iterator tti =
            (*pp)->root()->tcBegin();
  ...
  FunctionStructure::FunctionStructure() : levels(0), othercounts(1, 0) {
    for (pddl_type_list::const_iterator i =
             current_analysis->the_domain->types->begin();
         i != current_analysis->the_domain->types->end(); ++i) {
      if (theTC->isLeafType(*i)) {
        set< PropertySpace * > s = relevant(*i);
        if (!s.empty()) {
          ValueStructure vstr(*i);
          for (set< PropertySpace * >::iterator j = s.begin(); j != s.end();
               ++j) {
            if ((int)(theTC->range(*i).size()) >
                (*j)->oend() - (*j)->obegin()) {
              cout << "This space contains only a subset of the objects of the "
                      "type\n"
                   << "Suggest creating a sub-type\n";
            };

            ValuesUnion vu;
            for (PropertySpace::SIterator ps = (*j)->begin(); ps != (*j)->end();
                 ++ps) {
              Values vs;
  ...
  struct getMe {
    ...
    bool operator()(const pair< Property *, proposition * > &pp) {
       ...
       *		if(p==pp.first) return true;
                      ...
                      vector<Property*>::const_iterator j =
       std::find(ps.begin(),ps.end(),p); return j != ps.end();
      */
    };
  };

  int countRelevant(PropertySpace *ps,
                    const vector< pair< Property *, proposition * > > &conds) {
    int c = 0;
    for (vector< pair< Property *, proposition * > >::const_iterator i =
             conds.begin();
         i != conds.end(); ++i) {
  ...
  vector< ValueElement * > constructValue(
      const ValueStructure &vvs,
      const vector< pair< Property *, proposition * > > &conds,
      const vector< ValueElement * > &pvals) {
    vector< ValueElement * > vs;
    for (Range::const_iterator i = vvs.getRange().begin();
         i != vvs.getRange().end(); ++i) {
      // Now considering a state space
      ...
      for (ValuesUnion::const_iterator j = i->begin(); j != i->end(); ++j) {
        ...
        ps = (*k)->getState();
        //			int relevantConds =
        // countRelevant((*k)->getSpace(),conds);
        int cnt = 0;
        ...
        for (; k != j->end(); ++k) {
          ...
          // If the object is not in this state, we have to move on to the next
          // state. Object is probably in this state if it has the corresponding
          // initial property. However there is a possibility that the object is
          // in a different state that shares this property.
          //
          // If it has that then we can extract the initial proposition and the
          // arguments we need.
          //
          // If it doesn't have it, then we can suspect that it is not in this
          // state - but it might just be incompletely specified. The key is
          // that the object must be in some state and we know that all the
          // propositions will have to be accounted for by the state we
          // identify. So, we'll count how many propositions are accounted for
          // and then check that they were all handled by the end.
          Property *wanted = (*k)->getProp();
          if (!wanted) {
          ...
          // cout << "I want " << *wanted << "\n";
          // If the property is not wanted then we are looking at a special
          // value case. This is not a really bad problem for preconditions, but
          // it is much more of a problem for postconditions. To test it
          // properly we could use a trick: put the associated states before
          // this one, so that by the time we get here we know the associated
          // state will be already stored - that way we can find it and
          // correctly set this value! cout << "Want " << *wanted << "\n";
          ...
          if (fnd == conds.end()) {
            ...
            if (pvals.size() > vs.size() &&
                pvals[vs.size()]->size() > vals.size()) {
          ...
          if (got->args->size() == 1) {
            ...
            for (parameter_symbol_list::iterator pr = got->args->begin();
                 pr != got->args->end(); ++pr, ++c) {
              if (c != wanted->aPosn()) vals.push_back(new VarElement(*pr));
            };
          };
        };
        ...
        {
          // Then we found we were in this state, accounting for all
          // propositions, and so we can't be in any other, unless it shares the
          // common set of propositions. We could carry on and see if we can
          // find another match.
          //				if(cnt < (int)conds.size()) cout <<
          //"Still some conditions to account for\n";
          break;
        } else {
          for (vector< VElement * >::const_iterator xx = vals.begin();
               xx != vals.end(); ++xx)
            delete (*xx);
          vals.clear();
        };
  ...
  class ConditionGatherer : public VisitController {
   ...
   public:
    ...
    ConditionGatherer(const ConditionGatherer &cg)
        : gathered(cg.gathered.size()),
          values(cg.values),
          theStatics(cg.theStatics),
          others(){};
    ...
    virtual void visit_neg_goal(neg_goal *p) {
      cout << "Cannot handle negative preconditions yet (although should be "
              "able "
              "to manage !=)\n";
    };
    ...
    virtual void visit_proposition(proposition *p) {
      ...
      for (unsigned int i = 0; i < p->args->size(); ++i, ++ps) {
        ...
        // The following assumes that a property will be single valued for all
        // parameters (of any types) that may instantiate a particular position.
        // This looks for any one single valued property matching the property
        // from the proposition (which can be for a more general type than the
        ...
        for (vector< Property * >::const_iterator prp = ms.begin();
             prp != ms.end(); ++prp) {
          cout << "Considering " << **prp << "\n";
          if ((*prp)->isSingleValued()) {
            cout
                << "Think I should allocate this to parameter "
                << (static_cast< const IDsymbol< var_symbol > * >(*ps)->getId())
                << "\n";
            gathered[(static_cast< const IDsymbol< var_symbol > * >(*ps)
                          ->getId())]
    ...
    virtual void visit_effect_lists(effect_lists *p) {
      for (pc_list< simple_effect * >::iterator i = p->add_effects.begin();
           i != p->add_effects.end(); ++i) {
        (*i)->visit(this);
      };
    };
    void collect(const operator_ *op, FunctionStructure *fs, bool stateForAll,
                 VMap &valueFor) {
      ...
      for (var_symbol_list::const_iterator ps = op->parameters->begin();
        ...
        if (!fs->hasFluent((*ps)->type)) {
          ...
          for (vector< const pddl_type * >::const_iterator xx = tmptps.begin();
               xx != tmptps.end(); ++xx) {
            if (fs->hasFluent(*xx)) {
              //						cout << "Should
              // consider
          ...
          /* Generally we should ascend the entire hierarchy looking for
           * fluents.
           */
        ...
        for (vector< const pddl_type * >::const_iterator atp = tps.begin();
             atp != tps.end(); ++atp) {
          values.push_back(constructValue(
              fs->forType(*atp),
              gathered[(
                  static_cast< const IDsymbol< var_symbol > * >(*ps)->getId())],
              prevalues[values.size()]));
          int cc = 0;
          for (vector< ValueElement * >::const_iterator ve =
                   values[values.size() - 1].begin();
      ...
      SASOUTPUT {
        if (stateForAll && !theStatics.empty()) {
          cout << "\nStatic conditions:\n";
          for (vector< proposition * >::const_iterator s = theStatics.begin();
               s != theStatics.end(); ++s) {
            cout << "(" << (*s)->head->getName();
        ...
        if (!others.empty()) {
          ...
          for (vector< proposition * >::const_iterator s = others.begin();
               s != others.end(); ++s) {
            cout << "(" << (*s)->head->getName();
            for (parameter_symbol_list::const_iterator pm = (*s)->args->begin();
                 pm != (*s)->args->end(); ++pm) {
    ...
    SASActionTemplate *completeAction(operator_ *op, const VMap &pre,
                                      const VMap &post,
                                      ConditionGatherer &cg) const {
      return new SASActionTemplate(op, pre, post, theStatics, others,
                                   cg.others);
    };
  };

  void FunctionStructure::processActions() {
    for (operator_list::const_iterator i =
             current_analysis->the_domain->ops->begin();
         i != current_analysis->the_domain->ops->end(); ++i) {
      SASOUTPUT { cout << "===========\n" << *TAS((*i)->name) << "\n"; };
      /*		for(TIMactionSymbol::RCiterator r =
         TAS((*i)->name)->begin();r
         != TAS((*i)->name)->end();++r)
                      {
                              cout << **r << "\n";
  ...
  void FunctionStructure::buildLayers() {
    ...
    for (OpStore::iterator i = instantiatedOp::opsBegin();
         i != instantiatedOp::opsEnd(); ++i) {
  ...
  struct ValueStruct {
    ...
    ValueStruct(const pddl_type *p, const ValuesUnion &v,
                const PropertyState *s)
        : tp(p), vu(v), ps(s){};
  ...
  void FunctionStructure::normalise() {
    for (operator_list::const_iterator op =
             current_analysis->the_domain->ops->begin();
      ...
      for (var_symbol_list::const_iterator ps = (*op)->parameters->begin();
        ...
        for (TIMactionSymbol::RCiterator r = TAS((*op)->name)->begin();
          ...
          if ((*r)->paramNum() == c) {
            ...
            for (vector< const pddl_type * >::const_iterator tp = rtps.begin();
                 tp != rtps.end(); ++tp) {
              for (Range::const_iterator vu =
                       frep.find(*tp)->second.getRange().begin();
                ...
                if (v != (*vu).end()) {
                  if ((*r)->getLHS()->size() < v.forState()->size()) {
                    ...
                    //<<
                    //**r
                    //<< " applies to partial state description in "
                    //											<<
                    //*(v.forState()) << "\n";
        ...
        // The following block seems unnecessary because the action processing
        // handles the multiple subtypes case (although there could still be
        // unforeseen problems with that, so we'll keep this here for the
        // moment!
  ...
  ValuesUnion::ValuesUnion(const ValuesUnion &v1, const ValuesUnion &v2)
      : valuesUnion(v1.valuesUnion) {
    remove_copy_if(v2.valuesUnion.begin(), v2.valuesUnion.end(),
                   back_inserter(valuesUnion), alreadyIn(v1));
  };
  ...
  void ValueStructure::liftFrom(ValueStructure &vs1, ValueStructure &vs2) {
    Range r1, r2;
    for (Range::const_iterator i = vs1.getRange().begin();
         i != vs1.getRange().end(); ++i) {
  ...
  void FunctionStructure::restructure(const operator_ *op,
                                      const var_symbol *prm,
                                      const vector< const pddl_type * > &rtps) {
    SASOUTPUT {
      cout << "Looking for shared state structure in types: ";
      for (vector< const pddl_type * >::const_iterator i = rtps.begin();
           i != rtps.end(); ++i) {
        cout << (*i)->getName() << " ";
      };
      cout << "\n";
    ...
    if (rtps.size() > 2) {
      cerr << "Not sure how to handle so many sub-types for this abstraction "
              "process!\n"
  ...
  void FunctionStructure::restructure(const vector< ValueStruct > &toReduce,
                                      const vector< ValueStruct > &toLeave,
                                      const PropertyState *ps) {
    /*
            for(vector<ValueStruct>::const_iterator i = toLeave.begin();i !=
       toLeave.end();++i)
            {
       ...
       "\n";
            ...
            for(vector<ValueStruct>::const_iterator i = toReduce.begin();i !=
       toReduce.end();++i)
            {
    ...
    */
    ...
    if (!toLeave.empty()) {
      for (ValuesUnion::const_iterator i = toLeave[0].vu.begin();
           i != toLeave[0].vu.end(); ++i) {
        copy(i.forState()->begin(), i.forState()->end(),
             back_inserter(toSeparate));
      };
      //		cout << "Closure of properties: ";
      //		for_each(toSeparate.begin(),toSeparate.end(),ptrwriter<Property>(cout,"
      //")); 		cout << "\n";
    } else {
      //		cout << "Hmmm...we need to find the closure some other
      // way!\n";
      set< Property * > props;
    ...
    for (vector< ValueStruct >::const_iterator i = toReduce.begin();
         i != toReduce.end(); ++i) {
      ValuesUnion newvu1, newvu2;
      for (ValuesUnion::const_iterator j = i->vu.begin(); j != i->vu.end();
           ++j) {
        Values newvl1, newvl2;
        vector< Property * > prps1, prps2;
        //			cout << "Want to intersect with " <<
        //*(j.forState()) << "\n";
        for (PropertyState::PSIterator p = j.forState()->begin();
             p != j.forState()->end(); ++p) {
          // The j state is the one we want to split. The q values are the
          // properties we are trying to separate from the j state.
        ...
        if (!prps2.empty()) {
          ...
          reducedPSt =
              PropertyState::getPS(TA, i->tp, prps1.begin(), prps1.end());
          //				cout << "Split states into: " << *newpst
          //<< " and " << *reducedPSt << "\n";
      ...
      //		cout << "Planning to replace original ValuesUnion " <<
      // i->vu << " with " << newvu1 << " and "
      //				<< newvu2 << "\n";
      frep.find(i->tp)->second.update(i->vu, newvu2, newvu1);
    };
  };

  void Range::update(const ValuesUnion &oldvu, const ValuesUnion &newvu1,
                     const ValuesUnion &newvu2) {
  ...
  void ValueStructure::update(const ValuesUnion &oldvu,
                              const ValuesUnion &newvu1,
                              const ValuesUnion &newvu2) {
    range.update(oldvu, newvu1, newvu2);
  };

  bool ValuesUnion::hasPlaceHolder(const PropertyState *ps) {
    for (vector< pair< const PropertyState *, Values > >::iterator i =
             valuesUnion.begin();
         i != valuesUnion.end(); ++i) {
  ...
  void FunctionStructure::setUpInitialState() {
    ...
    for (pc_list< simple_effect * >::iterator i =
  ...
  bool FunctionStructure::growOneLevel() {
    ...
    for (Reachables::iterator i = reachables.begin(); i != reachables.end();
         ++i) {
      for (map< const TIMobjectSymbol *, RangeRep * >::iterator j =
               i->second.begin();
           j != i->second.end(); ++j) {
        j->second->cap();
      };
    ...
    for (int i = last; i < sz; ++i) {
      ...
      //"\n";
      for (extended_pred_symbol::OpProps::const_iterator j =
               EPS(others[i]->head)->posPresBegin();
        ...
        for (int k = startFor(j->op); k != endFor(j->op); ++k) {
          if (!(--(unsatisfiedPrecs[k]))) {
            ...
            sasActionTemplates[j->op]->enact(
                instantiatedOp::getInstOp(k)->getEnv(), reachables, others);
            activated = true;
          };
        };
      };
  ...
  ValueElement::ValueElement(ValueElement *vel, FastEnvironment *fe)
      : pst(vel->pst) {
    for (vector< VElement * >::iterator i = vel->value.begin();
         i != vel->value.end(); ++i) {
  ...
  bool FunctionStructure::tryMatchedPre(int k, instantiatedOp *iop,
                                        const var_symbol *var,
                                        SASActionTemplate *sasact,
                                        ValueRep *vrep) {
