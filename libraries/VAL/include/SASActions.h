...
namespace SAS {
  ...
  class ValueRep {
   ...
   public:
    ...
    void write(ostream &o) const { o << ptp->getName() << "_" << segment; };
  };
  ...
  class SegmentRep {
   ...
   public:
    ...
    bool add(ValueRep *vr, FastEnvironment *fe) {
      ...
      for (vector< ValueRep * >::iterator i = valuereps.begin();
           i != valuereps.end(); ++i) {
        if ((*i)->matches(vr, fe)) {
          //				cout << "nothing\n";
          return false;
      ...
      //		cout << "Added: " <<
      //*(valuereps[valuereps.size()-1]->getValue())
      //<<
      //"\n";
      return true;
    };
  ...
  class RangeRep {
   ...
   public:
    ...
    void cap() {
      for (vector< SegmentRep * >::iterator i = segreps.begin();
           i != segreps.end(); ++i) {
        (*i)->cap();
      };
    };
  ...
  typedef map<
      const pddl_type *,
      vector< vector< pair< const var_symbol *, SASActionTemplate * > > > >
      PreMap;
  ...
  class SASActionTemplate {
   ...
   public:
    SASActionTemplate(operator_ *o, const VMap &pres, const VMap &posts,
      ...
      for (VMap::const_iterator i = pres.begin(); i != pres.end(); ++i) {
        for (vector< ValueRep * >::const_iterator j = i->second.begin();
          ...
          preMap[i->first->type][(*j)->getSegment()].push_back(
              make_pair(i->first, this));
        };
      };
      for (vector< proposition * >::const_iterator i = opres.begin();
           i != opres.end(); ++i) {
    ...
    void write(ostream &o) const {
      ...
      o << ")\n  :precondition (and\n";
      for (VMap::const_iterator i = preconditions.begin();
           i != preconditions.end(); ++i) {
        for (vector< ValueRep * >::const_iterator j = i->second.begin();
             j != i->second.end(); ++j) {
      ...
      o << ")\n  :effect (and\n";
      for (VMap::const_iterator i = postconditions.begin();
           i != postconditions.end(); ++i) {
        for (vector< ValueRep * >::const_iterator j = i->second.begin();
             j != i->second.end(); ++j) {
          o << "      ((" << (*j)->typeName() << "_" << (*j)->getSegment()
            << " ?" << i->first->getName() << ") ";
    ...
    static vector< pair< const var_symbol *, SASActionTemplate * > > &findOps(
        const pddl_type *pt, int i) {
      return preMap[pt][i];
    };
    const operator_ *getOp() const { return op; };
    bool checkPre(FunctionStructure *fs, FastEnvironment *fenv,
