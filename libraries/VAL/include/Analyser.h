...
namespace VAL {
  ...
  class extended_pred_symbol : public pred_symbol {
   ...
   public:
    extended_pred_symbol(const string& nm)
        : pred_symbol(nm),
          initialState(0),
          goalState(0),
          preconds(),
    ...
    bool isStatic() const {
      return adds.empty() && derivedAdds.empty() && dels.empty();
    };
    bool decays() const {
      return adds.empty() && derivedAdds.empty() && !dels.empty();
    };
    ...
    void display(int i) const { write(cout); };
...
#define EPS(x) \
  ...
  class Analyser : public VisitController {
   ...
   public:
    Analyser()
        : initially(false), finally(false), adding(true), op(0), drv(0){};
