  ...
  class Property {
   ...
   public:
    ...
    void write(ostream &o) const {
      ...
      /*
                      o << "[";
                      for(vector<pddl_type *>::iterator i =
         EPS(predicate)->tBegin(); i != EPS(predicate)->tEnd();++i)
                      {
                              o << (*i)->getName() << " ";
                      };
                      o << "]";
  ...
  class TIMobjectSymbol : public VAL::const_symbol {
   ...
   public:
    ...
    VAL::proposition *find(const Property *p) const {
      ...
      return 0;
      /*
                      vector<Property*>::const_iterator i =
         std::find(initial.begin(),initial.end(),p); if(i == initial.end())
                      {
                              for(vector<Property*>::const_iterator ii =
         initial.begin();ii != initial.end();++ii)
                              {
                                      vector<Property*> ps = (*ii)->matchers();
                                      vector<Property*>::const_iterator j =
         std::find(ps.begin(),ps.end(),p); if(j != ps.end())
                                      {
                                              return
         initialps[ii-initial.begin()];
                                      };
                              };
                              return 0;
                      };
...
#define TOB(x) static_cast< TIM::TIMobjectSymbol * >(x)
  ...
  class PropertyState {
   ...
   public:
    ...
    PropertyState *adjust(const PropertyState *del, const PropertyState *add) {
      // Simple implementation is to remove del from this and then check that
      // the result found all entries in del. If so, union add and return a new
      // PropertyState, else return 0.
      //
      // There is probably an issue over types though. For example, if dels are
      // for a more specialised type than the entries in this what should
      // happen?
      ...
      if (ps.size() + del->properties.size() == properties.size()) {
        vector< Property * > qs;
        merge(ps.begin(), ps.end(), add->properties.begin(),
              add->properties.end(), inserter(qs, qs.begin()));
    ...
    PropertyState *add(TI s, TI e) {
      if (s == e) return this;
      vector< Property * > qs;
      merge(properties.begin(), properties.end(), s, e,
  ...
  struct recordIn {
    ...
    void operator()(Property *p) { p->addIn(ps); };
  };
  ...
  struct countInState {
    ...
    int operator()(PropertyState *ps) { return ps->count(prop); };
  };
  ...
  // states of the same (SV) property space, or if they define properties that
  // are different instantiations for the same object when only one instance of
  // the property appears in a state in a (SV) property space.
  ...
  class PropertySpace {
   ...
   public:
    ...
    void merge(PropertySpace *ps) {
      ...
      copy(ps->rules.begin(), ps->rules.end(), inserter(rules, rules.end()));
    ...
    vector< int > countsFor(Property *p) {
      vector< int > cs;
      transform(states.begin(), states.end(), inserter(cs, cs.begin()),
                countInState(p));
      return cs;
    };
  ...
  class TransitionRule {
   private:
    ...
    TransitionRule(TransitionRule *t, PropertyState *e, PropertyState *l,
                   PropertyState *r);
   ...
   public:
    ...
    bool isTrivial() const { return lhs->empty() && rhs->empty(); };
    bool isAttribute() const { return lhs->empty() || rhs->empty(); };
    bool isIncreasing() const { return lhs->empty() && !rhs->empty(); };
    bool isDecreasing() const { return rhs->empty() && !lhs->empty(); };
  ...
  struct ProtoRule {
    ...
    void insertPre(Property *p) { enablers.push_back(p); };
    void insertAdd(Property *p) { adds.push_back(p); };
    void insertDel(Property *p) { dels.push_back(p); };
  ...
  struct doExtension {
    ...
    void operator()(PropertySpace *p) { again |= p->extend(); };
  ...
  // Need this because mem_fun won't work with non-const methods like
  // sortObjects.
  ...
  class DurativeActionPredicateBuilder : public VAL::VisitController {
   ...
   public:
    ...
    virtual void visit_durative_action(VAL::durative_action *p) {
      ...
      if (inserting) {
        pred_symbol *nm =
            current_analysis->pred_tab.symbol_put(p->name->getName());
        ...
        es->add_effects.push_front(
            new simple_effect(new proposition(nm, p->parameters)));
        timed_effect *te = new timed_effect(ee, E_AT_END);
        ee->del_effects.push_front(
            new simple_effect(new proposition(nm, p->parameters)));
        p->effects->timed_effects.push_front(ts);
        p->effects->timed_effects.push_front(te);
        timed_goal *tg = new timed_goal(
            new simple_goal(new proposition(nm, p->parameters), E_POS),
            E_OVER_ALL);
        if (p->precondition) {
  ...
  class TIMactionSymbol : public VAL::operator_symbol {
   ...
   public:
    TIMactionSymbol(const string &nm)
        : operator_symbol(nm), fixedDuration(false){};
    ...
    bool isFixedDuration() const { return fixedDuration; };
    void assertFixedDuration() { fixedDuration = true; };
  };
...
#define TASc(x) static_cast< const TIM::TIMactionSymbol *const >(x)
  ...
  class TIMAnalyser : public VAL::VisitController {
   ...
   public:
    ...
    virtual void visit_durative_action(VAL::durative_action *p) {
      ...
      // that we can tell whether we generated from a start or end point.
      //
      // The tricky bit is the linkage: we need record invariants and we also
      // need to ensure that we don't lose state change across durative actions.
      // Toni's idea is to have a dummy add effect at start that is
      // preconditioned and deleted at the end. That should work, but needs a
      // couple of tweaks: 1: If the action has several state change effects
      // then this technique will leave
      //    them linked together in one state space. Not entirely clear how
      //    these could be split, but maybe we can use a technique that
      //    generalises the state splitting idea (ab->c, c->ab becomes a->c,
      //    c->a and b->c', c'->b).
      // 2: The mechanism artificially creates increasing/decreasing effects if
      // there was
      //    actually no property that was unlinked before introducing the
      //    dummies. We can filter these cases out, I think.
      //
    ...
    virtual void visit_problem(VAL::problem *p) {
      ...
      OUTPUT1 {
        for_each(trules.begin(), trules.end(),
                 ptrwriter< TransitionRule >(cout, "\n"));
      ...
      attrspaces =
          for_each(attrspaces.begin(), attrspaces.end(), doExamine(this));
  ...
  struct pairWith {
    ...
    mRec operator()(Property *o) { return mRec(o, v, oo); };
  };
...
#define TDA(x) dynamic_cast< TIM::TIMdurativeAction * >(x)
  ...
  struct mutRec {
    ...
    bool operator==(const mutRec &m) const {
      return first == m.first && second == m.second && one == m.one &&
             other == m.other;
    };

    bool operator!=(const mutRec &m) const { return !(*this == m); };
    ...
    bool operator<(const mutRec &m) const {
      return (first < m.first ||
              (first == m.first &&
               (second < m.second ||
                (second == m.second &&
                 (one < m.one || (one == m.one && other < m.other))))));
    };
  };
   ...
   public:
    ...
    static void constructMutex(VAL::operator_ *o1, int a1, VAL::operator_ *o2,
      ...
      OUTPUT cout << "Adding a mutex between " << o1->name->getName() << ":"
                  << a1 << " and " << o2->name->getName() << ":" << a2 << "\n";
      mutex *m = MEX(o1)->getMutex(o2);
      // This condition ensures that we don't store the same mutex pair in both
      // orders when they are arguments for the same operator. This is good if
      // we are going to use the pairs for checking, but it could be less good
      // if we want to check whether a specific pair of op-args are mutex.
      if (o1 == o2) {
        int a = min(a1, a2);
        if (a2 == a) {
          opType t3 = t1;
    ...
    unsigned int getMutexes(VAL::operator_ *A, TI sa, TI ea, TI sb, TI eb) {
      ...
      for (set< mutRec >::const_iterator i = argPairs.begin();
        ...
        if (*(getIx(sa, i->first)) == *(getIx(sb, i->second))) {
          //			cout << "Mutex for " << i->first << " " <<
          //**(getIx(sa,i->first)) << " and " << i->second
          //					<< " " <<
          //**(getIx(sb,i->second))
          //<<
          //"
          //"
          //<< i->one << " " << i->other << " " <<
          // 3*(i->one?i->one:1)+(i->other?i->other:1)
          //					<< "\n";
          ms |= 1 << 3 * (i->one ? i->one - 1 : 0) +
                         (i->other ? i->other - 1 : 0);
        };
      };
      return ms;
