...
class Agents {
 ...
 public:
  ...
  vector< int > whichGroups(const VAL::Environment &bdgs) {
    vector< int > gps;
    int c = 0;
    for (vector< vector< VAL::const_symbol * > >::iterator i = agentGps.begin();
         i != agentGps.end(); ++i, ++c) {
      for (vector< VAL::const_symbol * >::iterator j = i->begin();
           j != i->end(); ++j) {
        for (VAL::Environment::const_iterator k = bdgs.begin(); k != bdgs.end();
             ++k) {
          if (k->second == *j) {
            gps.push_back(c);
            j = i->end();
