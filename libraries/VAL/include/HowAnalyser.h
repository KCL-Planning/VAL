  ...
  class HWWAction {
   ...
   public:
    ...
    void write(ostream &o) const {
      for (ArgSets::const_iterator i = argSets.begin(); i != argSets.end();
           ++i) {
        o << "{";
    ...
    void analyse(action *a) {
      for (ArgSets::const_iterator i = argSets.begin(); i != argSets.end();
        ...
        for (goal_list::const_iterator g = cg->getGoals()->begin();
             g != cg->getGoals()->end(); ++g) {
  ...
  class HowAnalyser : public VisitController {
   ...
   public:
    ...
    virtual void visit_pred_decl(pred_decl *p) {
      ...
      if (hps) {
        for (holding_pred_symbol::PIt i = hps->pBegin(); i != hps->pEnd();
             ++i) {
          (*i)->writeName(cout);
          cout << ": ";
          if ((*i)->isStatic()) {
            ...
            for (extended_pred_symbol::OpProps::const_iterator j =
              ...
              vector< int > ags;
              cout << "\n\t" << *(j->op->name) << "(";
              for (var_symbol_list::const_iterator k =
                       j->op->parameters->begin();
                   k != j->op->parameters->end(); ++k, ++pc) {
                unsigned int ac = 0;
                bool d = false;
                parameter_symbol_list::const_iterator x =
                    j->second->args->begin();
                ...
                if (!d) {
                  ...
                  if ((*k)->type) {
                    ...
                    if (!ls.empty()) {
                      cout << " [";
                      for (vector< const pddl_type * >::const_iterator x =
                               ls.begin();
                           x != ls.end(); ++x) {
                        cout << (*x)->getName() << " ";
                      };
